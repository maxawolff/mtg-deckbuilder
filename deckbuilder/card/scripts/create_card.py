"""Method for creating cards of a given set."""
...
def gen_set(set_id, set_obj):
    ...
    for card in cards:
        ...
        if 'b' in card.number:
            ...
            new_card.save()
            cards_so_far = Card.objects.filter(from_set=set_obj)
            new_number = card.number[0:-1] + 'a'
            previous_card = cards_so_far.filter(number=new_number).first()
            previous_card.back_side = new_card
            import pdb; pdb.set_trace()
            previous_card.save()
        new_card.save()
